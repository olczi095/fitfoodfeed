    ...
    if width < min_width:
        if height < min_height:
            raise ValidationError(
                f"Image height and width are too small."
            )
        elif height > max_height:
            raise ValidationError(
                f"Image width is too small and image height is too large."
            )
        else:
            raise ValidationError(
                f"Image width is too small."
            )
    elif width > max_width:
        if height > max_height:
            raise ValidationError(
                f"Image height and width are too large."
            )
        elif height < min_height:
            raise ValidationError(
                f"Image width is too large and image height is too small."
            )
        else:
            raise ValidationError(
        ...
        raise ValidationError(
            f"Image height is too small."
        )
    elif height > max_height:
        raise ValidationError(
            f"Image height is too large."
        )